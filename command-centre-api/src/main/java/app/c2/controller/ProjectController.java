...
import app.c2.controller.exception.MissingFieldExceptionResponse;
...
public class ProjectController {
  ...
  @Autowired private MultipartFileStorageService multipartFileStorageService;
  ...
  public File saveFile(@PathVariable long projectId, @RequestParam(name = "remoteUrl") String remoteUrl, @RequestParam(name = "branch") String branch, @RequestParam(name = "path") String path) throws IOException, GitAPIException {
    app.c2.model.File file = fileStorageService.addGitFileToProject(remoteUrl,branch,path,projectId,null);
  ...
  public List<File> saveFiles(@PathVariable long projectId, @RequestParam(name = "remoteUrl") String remoteUrl, @RequestParam(name = "branch") String branch, @RequestParam(name = "paths") String paths) throws IOException, GitAPIException {
    ...
    for(String path: paths.split(",")){
      File file = null;
      file = fileStorageService.addGitFileToProject(remoteUrl,branch,path,projectId,null);
      file.setFileBlob(null);
      list.add(file);
    }
    return list;
  ...
  public File uploadFile(@RequestParam("file") MultipartFile multipartFile, @PathVariable("projectId") long projectId)  {
    ...
    File file;
    try {
      file = multipartFileStorageService.addUploadedFileToProject(multipartFile, projectId, null);
      file.setFileBlob(null);
      return file;
    } catch (IOException e) {
  ...
  public Set<File> getFilesInProject(@PathVariable long projectId) throws JsonProcessingException {
    return projectService.getFiles(projectId, null);
  }
  ...
  public void deleteFileFromProject(@PathVariable Long projectId, @PathVariable String fileName) throws JsonProcessingException {
    projectService.deleteFile(projectId, fileName,null);
  }
